---
nombre: Unicorn Gundam
designacion: RX-0
universo: uc
faccion: federacion-terrestre
pilotos: [banagher-links]
primera_aparicion: gundam-unicorn
fabricante: Anaheim Electronics (Proyecto UC)
tipo: Mobile suit de armazón psicoframe completo
especificaciones:
  - { etiqueta: "Universo", valor: "Universal Century" }
  - { etiqueta: "Primera aparición", valor: "Mobile Suit Gundam Unicorn, 2010" }
  - { etiqueta: "Sistema", valor: "NT-D (Newtype Destroyer)" }
  - { etiqueta: "Modos", valor: "Unicorn y Destroy" }
resumen: Un mobile suit blanco de un solo cuerno que se transforma al detectar un Newtype. La máquina que devolvió al UC su icono.
relacionados: [msz-006-zeta-gundam, rx-93-nu-gundam]
kits: []
codigo: MS-UC-RX-0
fuentes:
  - https://en.wikipedia.org/wiki/Mobile_Suit_Gundam_Unicorn
actualizado: '2026-09-05'
---

El RX-0 Unicorn Gundam es el primer mobile suit con armazón de psicoframe completo, desarrollado por Anaheim
Electronics dentro del "Proyecto UC" y entregado por la Fundación Vist. En modo Unicorn es una máquina blanca,
lisa, con un solo cuerno; cuando el sistema NT-D detecta a un Newtype, el armazón se expande y el cuerno se
abre en la V característica de los Gundams, revelando el psicoframe iluminado.

Esa transformación —de máquina anónima a Gundam— es el argumento visual de toda la serie, y la razón por la que
el Unicorn se convirtió en el nuevo emblema del Universal Century: es el mobile suit de la estatua a escala
real de Odaiba, en Tokio.

En Gunpla, las versiones Master Grade, Real Grade y Perfect Grade reproducen la transformación completa, con
distinto nivel de exigencia. El Unicorn en Destroy Mode es uno de los kits más fotografiados del hobby.
