---
nombre: Hero Zone
descripcion_corta: Tienda de coleccionables en Tijuana con dos puntos de venta, envío nacional de tarifa plana y colección etiquetada de Gundam.
esquema_version: 2
veredicto: 'De las pocas referencias verificables en Baja California, con domicilio y teléfono publicados. Su surtido de Gunpla parece ocasional: no lo tomes como catálogo profundo.'
tipo: hibrida
categoria: coleccionables
sucursales:
  - etiqueta: Playas de Tijuana
    calle: Av. Paseo Ensenada 130-A004
    colonia: Playas de Tijuana Sección Coronado
    ciudad: Tijuana
    estado: baja-california
    cp: '22504'
    telefono: '6649773674'
  - etiqueta: Plaza Río
    ciudad: Tijuana
    estado: baja-california
web: https://www.herozone.com.mx/
redes:
  facebook: https://www.facebook.com/herozonemx
  instagram: https://www.instagram.com/herozonemx/
  youtube: https://www.youtube.com/@herozone6205
  tiktok: https://www.tiktok.com/@herozonetijuana
vende_gunpla: ocasional
maneja_preventa: true
origen_producto: no_verificado
grados: [hg]
envio_nacional: true
pickup: true
pagos: [Visa, Mastercard, Maestro, PayPal, Apple Pay, Amazon Pay, Shop Pay]
evidencias:
  - afirmacion: Tiene colección etiquetada de Gundam, aunque su foco es el coleccionable pop en general
    url: https://www.herozone.com.mx/collections/all/gundam
    cita: Colección con etiqueta Gundam
    fecha: '2026-09-04'
    vigencia_meses: 3
  - afirmacion: Su punto de Playas de Tijuana está en Av. Paseo Ensenada 130-A004, Sección Coronado, 22504 Tijuana
    url: https://www.herozone.com.mx/
    cita: Pie de página
    fecha: '2026-09-09'
    vigencia_meses: 12
  - afirmacion: Publica el teléfono 664 977 3674
    url: https://www.herozone.com.mx/
    cita: Pie de página
    fecha: '2026-09-09'
    vigencia_meses: 12
  - afirmacion: Envío a todo México con tarifa plana de 190 pesos
    url: https://www.herozone.com.mx/
    cita: Aviso de envío de la portada
    fecha: '2026-09-09'
    vigencia_meses: 3
  - afirmacion: Acepta Visa, Mastercard, Maestro, PayPal, Apple Pay, Amazon Pay y Shop Pay
    url: https://www.herozone.com.mx/
    cita: Pie de página
    fecha: '2026-09-09'
    vigencia_meses: 3
  - afirmacion: Publica perfiles de Facebook, Instagram, YouTube y TikTok
    url: https://www.herozone.com.mx/
    cita: Pie de página
    fecha: '2026-09-09'
    vigencia_meses: 12
  - afirmacion: Publica productos en preventa dentro de su catálogo
    url: https://www.herozone.com.mx/
    cita: Productos rotulados PREVENTA en la portada
    fecha: '2026-09-09'
    vigencia_meses: 6
  - afirmacion: No publica horarios de atención ni el domicilio de su punto de Plaza Río
    url: https://www.herozone.com.mx/
    cita: Ausencia comprobada en portada y pie de página
    fecha: '2026-09-09'
    vigencia_meses: 6
faq:
  - pregunta: ¿Hero Zone tiene Gunpla en existencia?
    respuesta: >-
      No lo podemos afirmar. Tiene una colección etiquetada como Gundam, pero su foco es el
      coleccionable pop en general y no confirmamos profundidad ni rotación. Por eso esta ficha declara
      su oferta de Gunpla como ocasional y no como línea establecida. Pregunta por teléfono antes de ir.
  - pregunta: ¿Cuánto cuesta el envío?
    respuesta: >-
      Al 9 de septiembre de 2026 su portada anunciaba tarifa plana de 190 pesos a todo México, sin
      umbral de envío gratis. No publica con qué paqueterías envía.
  - pregunta: ¿Dónde están sus tiendas?
    respuesta: >-
      Publica el domicilio completo de su punto de Playas de Tijuana, en Av. Paseo Ensenada 130-A004,
      Sección Coronado, C.P. 22504, con teléfono. De su punto de Plaza Río no publica domicilio ni
      horarios, así que conviene confirmarlo antes de ir.
verificacion:
  estado: sin_verificar
  fecha: '2026-09-09'
  metodo: sitio_web
  nivel_confianza: 1
  fuentes:
    - https://www.herozone.com.mx/
    - https://www.herozone.com.mx/collections/all/gundam
  notas: >-
    La ficha se mantiene en "sin_verificar" con confianza 1 a propósito: los datos de contacto,
    domicilio, envío y pagos sí están comprobados y fechados en el expediente, pero lo que NO se pudo
    comprobar es lo que el usuario viene a preguntar, que es si maneja Gunpla de forma regular y con
    qué origen. Tener buenos datos de contacto no vuelve verificada la oferta.
alta: '2026-09-04'
actualizada: '2026-09-09'
---

Hero Zone es una tienda de coleccionables de Tijuana con dos puntos de venta y venta en línea. Aparece
en este directorio porque es de las pocas referencias verificables en Baja California, un estado donde
la oferta de Gunpla es delgada, pero aparece con la advertencia puesta.

Lo que sí está comprobado y fechado: el domicilio completo de su punto de Playas de Tijuana, con código
postal y teléfono; envío a todo México con tarifa plana de 190 pesos; siete formas de pago, incluidas
Apple Pay, Amazon Pay y Shop Pay; cuatro redes sociales activas; y productos publicados en preventa. En
datos de contacto está mejor documentada que varias fichas del directorio.

Lo que no está comprobado es justo lo que importa para decidir una compra de Gunpla. Tiene una
colección etiquetada como Gundam, pero su catálogo es de coleccionable pop en general, no confirmamos
la profundidad del surtido ni su rotación, y no publica documento ni declaración del canal por el que
compra. Por eso su oferta se declara como ocasional, la verificación se queda en "sin verificar" y el
nivel de confianza en uno de tres. Tampoco publica horarios ni el domicilio de su segundo punto, en
Plaza Río. Si vas por un kit concreto, háblales antes al teléfono que publican.
