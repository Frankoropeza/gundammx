---
nombre: Distrito Max
descripcion_corta: Tienda en línea de coleccionables con colección dedicada a Bandai Hobby, envío gratis desde 700 pesos y recogida en su bodega de Naucalpan.
esquema_version: 2
veredicto: 'Su umbral de envío gratis es de los más bajos del directorio. No es una tienda que se pueda visitar: su propio sitio dice que no tiene sucursales, sólo una bodega para recoger.'
tipo: hibrida
categoria: coleccionables
sucursales:
  - etiqueta: Bodega de recogida
    calle: Calle Norte 4
    colonia: Alce Blanco
    # La ciudad se conserva como "Naucalpan" porque de ella deriva el slug de
    # /tiendas/ciudad/naucalpan/, que ya está publicada. El nombre municipal
    # completo va en municipio y en el domicilio del expediente.
    ciudad: Naucalpan
    municipio: Naucalpan de Juárez
    estado: estado-de-mexico
    cp: '53370'
    solo_recoleccion: true
web: https://www.distritomax.com/
vende_gunpla: si
maneja_preventa: true
origen_producto: no_verificado
grados: [hg, rg, mg]
envio_nacional: true
envio_gratis_desde: 700
pickup: true
pagos: [Visa, Mastercard, American Express, PayPal, Apple Pay, Google Pay, Diners Club, Discover, JCB]
evidencias:
  - afirmacion: Tiene colección específica de Bandai Hobby Gunpla, con High Grade, Real Grade y Master Grade
    url: https://www.distritomax.com/collections/bandai-hobby-gunpla
    cita: Colección de Bandai Hobby Gunpla, censo inicial
    fecha: '2026-09-04'
    vigencia_meses: 3
  - afirmacion: Envía a todo el país, con envío gratis a partir de 700 pesos
    url: https://www.distritomax.com/
    cita: Aviso de envío de la portada
    fecha: '2026-09-08'
    vigencia_meses: 3
  - afirmacion: 'Su propio sitio declara que no tiene sucursales físicas: sólo una bodega donde recoger lo comprado en línea'
    url: https://www.distritomax.com/pages/sucursales
    cita: Página de sucursales
    fecha: '2026-09-08'
    vigencia_meses: 12
  - afirmacion: La bodega de recogida está en Calle Norte 4, Alce Blanco, 53370 Naucalpan de Juárez
    url: https://www.distritomax.com/pages/sucursales
    cita: Página de sucursales
    fecha: '2026-09-08'
    vigencia_meses: 12
  - afirmacion: Acepta nueve formas de pago, entre ellas Apple Pay, Google Pay, Diners Club, Discover y JCB
    url: https://www.distritomax.com/
    cita: Pie de página
    fecha: '2026-09-08'
    vigencia_meses: 3
  - afirmacion: Tiene tres secciones de preventa en su navegación
    url: https://www.distritomax.com/
    cita: 'Navegación: PREVENTAS, PREVENTAS GOD y PREVENTAS MÁS VENDIDAS'
    fecha: '2026-09-08'
    vigencia_meses: 6
  - afirmacion: No publica redes sociales, teléfono, correo ni horarios de atención
    url: https://www.distritomax.com/
    cita: Ausencia comprobada en portada y pie de página
    fecha: '2026-09-08'
    vigencia_meses: 6
faq:
  - pregunta: ¿Puedo ir a Distrito Max a ver los kits?
    respuesta: >-
      No. Su propia página de sucursales dice que no tiene tiendas físicas: lo que hay en Calle Norte
      4, colonia Alce Blanco, Naucalpan, es una bodega para recoger lo que compraste en línea. No
      publica horarios, así que conviene confirmar antes de ir.
  - pregunta: ¿Desde cuánto sale gratis el envío?
    respuesta: >-
      Desde 700 pesos al 8 de septiembre de 2026, que es de los umbrales más bajos que registramos en
      el directorio. No publica con qué paqueterías envía ni tiempos de entrega.
  - pregunta: ¿Cómo contacto a Distrito Max si hay un problema con mi pedido?
    respuesta: >-
      Ahí está su punto débil: no publica teléfono, correo, WhatsApp ni redes sociales en su sitio.
      El único canal visible es su propia tienda en línea.
verificacion:
  estado: verificada
  fecha: '2026-09-08'
  metodo: sitio_web
  nivel_confianza: 2
  fuentes:
    - https://www.distritomax.com/
    - https://www.distritomax.com/pages/sucursales
  notas: >-
    Corrección importante del 2026-09-08: la ficha describía un "almacén y outlet" como sucursal. Su
    página de sucursales declara que NO tiene tiendas físicas, sólo una bodega de recogida, así que la
    dirección se marca con solo_recoleccion y no se emite marcado de negocio local. La colección de
    Bandai Hobby viene del censo del 2026-09-04. Su política de envío no detalla paqueterías ni
    tiempos. origen_producto queda en "no_verificado".
alta: '2026-09-04'
actualizada: '2026-09-08'
---

Distrito Max es una operación de comercio en línea con colección dedicada a Bandai Hobby dentro de un
catálogo de coleccionables más amplio. Le sirve a quien compra por monto y quiere que el envío no se
coma la diferencia, y a quien vive en el norponiente del Valle de México y prefiere recoger.

Su ventaja concreta es el umbral: envío gratis desde 700 pesos, de los más bajos que registramos en el
directorio, que en un kit High Grade cambia el total de forma notoria. Acepta nueve formas de pago,
incluidas Apple Pay, Google Pay, Diners Club, Discover y JCB. Y tiene tres secciones distintas de
preventa en su navegación, lo que sugiere que es una parte importante de su operación.

Ahora la parte que hay que entender bien antes de planear una visita. **Su propia página de sucursales
dice que no tiene tiendas físicas.** Lo que existe en Calle Norte 4, colonia Alce Blanco, Naucalpan, es
una bodega para recoger lo que ya compraste en línea, no un local donde entrar a ver kits. Por eso esta
ficha marca esa dirección como punto de recolección y no como tienda. Sumado a eso, no publica
teléfono, correo, WhatsApp ni redes sociales, así que si un pedido sale mal el único canal es su propia
tienda. Tampoco publica paqueterías ni tiempos de entrega.
